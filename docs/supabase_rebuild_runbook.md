...
6. `docs/security_guest_rsvp_validation.sql`
7. `docs/security_guest_gift_payment_validation.sql`
8. `docs/security_invite_code_generation.sql`
9. `docs/security_admin_gift_operations.sql`
10. `docs/security_admin_rsvp_operations.sql`
11. `docs/security_admin_guest_operations.sql`
12. `docs/security_admin_gift_catalog_operations.sql`
13. `docs/security_admin_settings_operations.sql`
14. `docs/security_public_settings_access.sql`
15. `docs/security_edge_function_service_role_grants.sql`
16. `docs/security_phase_4_rls_activate.sql`
17. `docs/supabase_rebuild_verify_final.sql`
...
docs/security_guest_rsvp_validation.sql
docs/security_guest_gift_payment_validation.sql
docs/security_invite_code_generation.sql
docs/security_admin_gift_operations.sql
docs/security_admin_rsvp_operations.sql
docs/security_admin_guest_operations.sql
docs/security_admin_gift_catalog_operations.sql
docs/security_admin_settings_operations.sql
docs/security_public_settings_access.sql
...
políticas administrativas, geração segura dos códigos de convite e permissões
internas da Edge Function. As confirmações e liberações administrativas de
presentes e cotas, além da criação, atualização e remoção administrativa de
RSVPs, também passam a ocorrer por RPCs transacionais. A edição e a mudança de
estado dos convidados ficam restritas às RPCs administrativas, com revogação
das sessões quando um convite é desativado. O catálogo de presentes também é
criado, validado, atualizado e excluído exclusivamente por RPCs. As
configurações globais são validadas por uma RPC e limitadas a um único registro.
A leitura dessas configurações também ocorre por uma RPC com colunas fixas; o
frontend não possui acesso direto à tabela `settings`, evitando que colunas
adicionadas futuramente sejam expostas automaticamente.
Nomes dos noivos, data do casamento, prazo do RSVP e dados da cerimônia e da
recepção também são mantidos nesse registro e alimentam dinamicamente as
páginas públicas.

O RSVP público reconstrói os dados a partir do convite armazenado, valida os
membros do casal, limita os acompanhantes e aceita somente as idades
padronizadas exibidas no formulário.

Nos presentes individuais, a forma escolhida deve ser compatível com o modo
de compra cadastrado. A informação de pagamento ou compra só é aceita depois
que uma forma válida foi registrada para aquele presente.

A criação de convidados no painel usa a RPC
`create_guest_with_invite_code`. Ela valida a conta administrativa e gera no
banco um código aleatório de oito caracteres com `pgcrypto`. O frontend não
deve gerar nem enviar `invite_code` em novos cadastros.
...
Para um projeto criado antes das configurações dinâmicas do evento, execute:

```text
docs/wedding_event_settings_migration.sql
docs/security_admin_settings_operations.sql
docs/security_public_settings_access.sql
docs/wedding_event_settings_verify.sql
docs/security_admin_settings_operations_verify.sql
docs/security_public_settings_access_verify.sql
```
